...
import com.nttdata.messanger.Exception.DataNotFoundException;
import com.nttdata.messanger.database.DatabaseClass;
import com.nttdata.messanger.model.Message;

public class MessageService {
	public MessageService() {
		messages.put(1L, new Message(1L, "Hello World", "Himanshu"));
		messages.put(2L, new Message(2L, "Hello RestWeb", "Prajapati"));
	...
	public List<Message> getMessagesPaginated(int start, int size) {
		...
		System.out.println(list.subList(start - 1, size + start - 1));
		return list.subList(start - 1, size + start - 1);
	}
